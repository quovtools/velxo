...
let RewardsController = class RewardsController {
    ...
    async getCoinBalance(userId) {
        ...
        return api_response_dto_1.ApiResponseDto.ok(data, 'Coin balance retrieved');
    }
    async getTransactions(userId, limit) {
        const data = await this.rewardsService.getTransactions(userId, limit ? parseInt(limit) : 50);
        return api_response_dto_1.ApiResponseDto.ok(data, 'Transactions retrieved');
    }
    async getCatalog() {
        const data = await this.rewardsService.getCatalog();
        return api_response_dto_1.ApiResponseDto.ok(data, 'Reward catalog retrieved');
    }
    async redeem(userId, catalogId) {
        const data = await this.rewardsService.redeem(userId, catalogId);
        return api_response_dto_1.ApiResponseDto.ok(data, 'Reward redeemed successfully');
    }
};
...
], RewardsController.prototype, "getCoinBalance", null);
__decorate([
...
__decorate([
    ...
    (0, common_1.UseGuards)(supabase_jwt_guard_1.SupabaseJwtGuard),
    __param(0, (0, current_user_decorator_1.CurrentUserId)()),
    __param(1, (0, common_1.Body)('catalogId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
