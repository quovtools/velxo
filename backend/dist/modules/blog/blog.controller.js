...
let BlogController = class BlogController {
    ...
    async getPosts(category) {
        ...
        return api_response_dto_1.ApiResponseDto.ok(posts, 'Posts retrieved');
    }
    async getAllPosts() {
        const posts = await this.blogService.getAllPosts();
        return api_response_dto_1.ApiResponseDto.ok(posts, 'All posts retrieved');
    }
    async getPost(slug) {
        const post = await this.blogService.getPostBySlug(slug);
        return api_response_dto_1.ApiResponseDto.ok(post, 'Post retrieved');
    }
    async createPost(dto) {
        const post = await this.blogService.createPost(dto);
        return api_response_dto_1.ApiResponseDto.ok(post, 'Post created');
    }
    async updatePost(id, dto) {
        const post = await this.blogService.updatePost(id, dto);
        return api_response_dto_1.ApiResponseDto.ok(post, 'Post updated');
    }
    async deletePost(id) {
        await this.blogService.deletePost(id);
        return api_response_dto_1.ApiResponseDto.ok(null, 'Post deleted');
    }
};
