...
let EmailService = EmailService_1 = class EmailService {
    constructor() {
        ...
        if (!apiKey) {
            this.logger.error('RESEND_API_KEY is NOT set. Emails will NOT be sent. Set RESEND_API_KEY in your environment.');
        }
    ...
    async sendEmail(to, subject, html) {
        if (!this.resend) {
            this.logger.error(`[EMAIL NOT SENT] RESEND_API_KEY missing. Would have sent "${subject}" to ${to}`);
    ...
    async sendOrderConfirmationEmail(email, orderNumber, sellerName, totalAmount, items) {
    ...
    async sendWalletTransactionEmail(email, transactionType, amount, balanceAfter, description) {
    ...
    async sendNotificationEmail(email, title, body) {
        const html = `
      <!DOCTYPE html>
      <html>
      <head>
