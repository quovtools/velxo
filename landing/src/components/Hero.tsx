...
const TRUST = [
  { icon: <ShieldCheck className="h-3.5 w-3.5 text-emerald-400" />, text: 'Escrow on every trade' },
  { icon: <Zap className="h-3.5 w-3.5 text-brand" />,              text: 'Instant delivery' },
  { icon: <Star className="h-3.5 w-3.5 text-brand" />,             text: 'Verified sellers' },
  { icon: <TrendingUp className="h-3.5 w-3.5 text-brand-light" />, text: 'Trusted across Africa' },
];
...
export default function Hero() {
  return (
    <section aria-labelledby="hero-heading" className="relative flex min-h-screen items-center justify-center overflow-hidden pt-16">
      {/* Subtle gold radial on a pure black background */}
      ...
      {/* Very restrained gold glow blobs — barely visible */}
      <div className="absolute -top-24 left-1/4 h-96 w-96 rounded-full bg-brand/8 blur-3xl" />
      <div className="absolute -bottom-24 right-1/4 h-72 w-72 rounded-full bg-brand/6 blur-3xl" />
      ...
      <div className="container-x relative py-24 text-center">
        ...
        <h1 id="hero-heading" className="mx-auto max-w-4xl text-5xl font-black leading-[1.05] tracking-tight text-white sm:text-6xl md:text-7xl lg:text-8xl">
          Trade Games.{' '}
          {/* Gold gradient text — the only heavy color use on this page */}
        ...
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          ...
          {/* Secondary: transparent with neutral border */}
