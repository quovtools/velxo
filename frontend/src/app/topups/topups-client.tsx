  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none -mx-1 px-1">
        <button
          onClick={() => { setActiveGame(''); fetchProducts(); }}
        ...
        {GAMES.map((g) => (
          <button
            key={g}
            onClick={() => { setActiveGame(g); fetchProducts(g); }}
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-64 bg-cardBg border border-borderBg rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : products.length === 0 ? (
        <div className="text-center py-20 bg-cardBg border border-borderBg rounded-3xl">
          ...
          <p className="text-gray-400 font-semibold">
            No top-ups available{activeGame ? ` for ${activeGame}` : ''}.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {products.map((p) => (
            <div
              key={p.id}
              className="bg-cardBg border border-borderBg hover:border-brand/40 rounded-2xl overflow-hidden flex flex-col transition"
            >
              <div className="h-32 bg-gradient-to-br from-brand/30 to-background flex items-center justify-center relative">
                {p.imageUrl ? (
                  <img src={p.imageUrl} alt={p.title} className="w-full h-full object-cover" />
                ) : (
                  <Zap className="w-8 h-8 text-brand/60" />
                )}
              ...
              <div className="p-3 flex-1 flex flex-col justify-between space-y-2">
                ...
                {p.deliveryInfo && (
                  <p className="text-[10px] text-gray-500 flex items-center gap-1">
                    <ShieldCheck className="w-3 h-3 text-emerald-400" /> {p.deliveryInfo}
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="bg-brand/5 border border-brand/20 rounded-2xl p-4 flex items-center gap-3 text-xs text-gray-400">
        ...
        All official top-ups are fulfilled by Piyrox and protected by escrow — you only release payment after delivery.
      </div>
          ...
          >
            <div className="relative h-36 bg-gradient-to-br from-brand/30 to-background flex items-center justify-center">
              {selected.imageUrl ? (
                ...
                <Zap className="w-10 h-10 text-brand/60" />
              )}
              <button
                onClick={closeModal}
              ...
              <span className="absolute top-3 left-3 text-[9px] font-bold uppercase bg-brand text-white px-1.5 py-0.5 rounded">
                Official
              </span>
            </div>
              ...
              <div className="p-6 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Your top-up order has been created and is protected by escrow.
                  </p>
                </div>
                ...
                <div>
                  ...
                  {selected.deliveryInfo && (
                    <p className="text-[11px] text-gray-500 flex items-center gap-1 mt-2">
                      <ShieldCheck className="w-3 h-3 text-emerald-400" /> {selected.deliveryInfo}
                    </p>
                  )}
                </div>
