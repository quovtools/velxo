  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-[var(--card-bg)] border border-[var(--border-bg)] rounded-2xl overflow-hidden">
                ...
                {(['images','video'] as const).map(tab => (
                  <button key={tab} onClick={() => setMediaTab(tab)}
                    className={`flex items-center gap-1.5 px-5 py-3 text-xs font-bold transition border-b-2 ${
                      mediaTab === tab ? 'border-brand text-brand' : 'border-transparent text-gray-500 hover:text-gray-300'
                    }`}>
                    {tab === 'images' ? <><Image className="w-3.5 h-3.5" /> Photos {images.length > 0 && `(${images.length})`}</> : <><Video className="w-3.5 h-3.5" /> Video</>}
                  </button>
                ))}
              </div>
            )}
            ...
            {(mediaTab === 'images' || !hasVideo) && (
              <>
                <div className="relative aspect-video sm:aspect-[16/10] lg:aspect-[4/3] bg-black flex items-center justify-center">
                  {images.length > 0 ? (
                    <>
                      ...
                      {images.length > 1 && (
                        <>
                          <button onClick={() => setImgIdx(i => (i - 1 + images.length) % images.length)}
                            className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/60 hover:bg-black/80 border border-white/10 rounded-full flex items-center justify-center transition">
                            <ChevronLeft className="w-4 h-4 text-white" />
                          </button>
                          <button onClick={() => setImgIdx(i => (i + 1) % images.length)}
                            className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-black/60 hover:bg-black/80 border border-white/10 rounded-full flex items-center justify-center transition">
                            <ChevronRight className="w-4 h-4 text-white" />
                          </button>
                          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                            {images.map((_, i) => (
                              <button key={i} onClick={() => setImgIdx(i)}
                                className={`w-2 h-2 rounded-full transition ${i === imgIdx ? 'bg-brand' : 'bg-white/30'}`} />
                            ))}
                          </div>
                        </>
                      )}
                  ...
                  <div className="flex gap-2 p-3 overflow-x-auto scrollbar-none bg-black/20">
                    {images.map((img, i) => (
                      ...
                      <img key={i} src={img} alt="" loading="lazy" referrerPolicy="no-referrer" onClick={() => setImgIdx(i)}
                        className={`h-14 w-20 object-cover rounded-lg cursor-pointer flex-shrink-0 transition border-2 ${
          ...
          {/* Title + metadata */}
          <div className="bg-[var(--card-bg)] border border-[var(--border-bg)] rounded-2xl p-6 space-y-4">
            <div className="flex flex-wrap gap-2">
              <span className="bg-brand/10 text-brand text-xs font-bold px-3 py-1 rounded-full border border-brand/20">{listing.gameName}</span>
              {listing.platform && <span className="bg-white/5 text-gray-300 text-xs font-semibold px-3 py-1 rounded-full border border-white/10 flex items-center gap-1"><Monitor className="w-3 h-3" />{listing.platform}</span>}
              {listing.region   && <span className="bg-white/5 text-gray-300 text-xs font-semibold px-3 py-1 rounded-full border border-white/10 flex items-center gap-1"><MapPin className="w-3 h-3" />{listing.region}</span>}
              {listing.deliveryTime != null && listing.deliveryTime === 0 && <span className="bg-brand/5 text-brand text-xs font-semibold px-3 py-1 rounded-full border border-brand/15 flex items-center gap-1"><Zap className="w-3 h-3" />Instant Delivery</span>}
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white leading-snug">{listing.title}</h1>

            {/* Account details grid */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'Rank',     val: listing.rank || 'N/A' },
                { label: 'Level',    val: listing.level ? `Level ${listing.level}` : 'N/A' },
                { label: 'Platform', val: listing.platform || 'Any' },
                { label: 'Delivery', val: listing.deliveryTime != null ? (listing.deliveryTime === 0 ? 'Instant' : `~${listing.deliveryTime} min`) : 'Instant' },
              ].map(m => (
                <div key={m.label} className="bg-black/30 border border-[var(--border-bg)] rounded-xl p-3">
                  <p className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">{m.label}</p>
                  <p className="text-sm font-bold text-white mt-0.5 truncate">{m.val}</p>
                </div>
              ))}
            </div>
            ...
            {/* Description */}
            <div className="pt-4 border-t border-[var(--border-bg)]">
          ...
          <div className="bg-[var(--card-bg)] border border-[var(--border-bg)] rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-white flex items-center gap-2"><Star className="w-4 h-4 text-brand" /> Seller Reviews</h3>
        ...
        {/* ── Right: Purchase card ── */}
        <div>
          <div className="bg-[var(--card-bg)] border border-[var(--border-bg)] rounded-2xl p-6 space-y-5 lg:sticky lg:top-20">

            {/* Price */}
            <div>
              <p className="text-[11px] text-gray-500 uppercase tracking-wider font-semibold mb-1">Price</p>
              <span className="text-4xl font-black text-white">{fmt(listing.price)}</span>
            </div>

            {/* Trust indicators */}
            <div className="space-y-2.5 border-y border-[var(--border-bg)] py-4">
              {[
                { icon: <Shield className="w-4 h-4 text-brand" />, text: 'Escrow protection on all orders' },
                { icon: <Clock className="w-4 h-4 text-brand" />, text: listing.deliveryTime != null && listing.deliveryTime === 0 ? 'Instant delivery' : `Delivery in ~${listing.deliveryTime} min` },
                { icon: <UserCheck className="w-4 h-4 text-brand" />, text: listing.seller?.isVerified ? 'Verified seller' : 'Registered seller' },
              ].map((t, i) => (
            ...
            <div className="space-y-2.5">
              {listing.isSold || listing.status === 'SOLD' ? (
                <div className="flex items-center justify-center gap-2 w-full bg-black/40 border border-[var(--border-bg)] py-3.5 rounded-xl font-bold text-gray-500 cursor-not-allowed">
                  <ShoppingCart className="w-4 h-4" /> Sold Out
                </div>
              ) : (
                <Link href={`/checkout/${listing.id}`}
                  className="flex items-center justify-center gap-2 w-full bg-brand hover:bg-brand-light py-3.5 rounded-xl font-bold transition shadow-lg shadow-brand/20 text-black text-sm">
                  <ShoppingCart className="w-4 h-4" /> Buy Now
                </Link>
              )}
              {user && (user as any).id !== listing.seller?.userId && (
                <Link href={`/messages?sellerId=${listing.seller?.userId}`}
                  className="flex items-center justify-center gap-2 w-full bg-black/30 border border-[var(--border-bg)] hover:border-brand/40 py-3.5 rounded-xl font-bold transition text-gray-300 hover:text-white text-sm">
            ...
            {/* Seller card */}
            <div className="border-t border-[var(--border-bg)] pt-4 space-y-3">
              <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Seller</p>
              <Link href={`/seller/${listing.seller?.id}`} className="flex items-center gap-3 group p-3 rounded-xl bg-black/20 border border-[var(--border-bg)] hover:border-brand/30 transition">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {listing.seller?.isOnline && (
                      <span className="text-[9px] bg-brand/10 text-brand border border-brand/20 px-1.5 py-0.5 rounded font-bold">● Online</span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-2">
                <Link href={`/seller/${listing.seller?.id}`}
                  className="flex-1 flex items-center justify-center gap-1.5 bg-black/20 border border-[var(--border-bg)] hover:border-brand/40 py-2.5 rounded-xl text-xs font-bold text-gray-300 hover:text-white transition">
                  <Store className="w-3.5 h-3.5" /> View Profile
                </Link>
                {user && (user as any).id !== listing.seller?.userId && (
                  <button onClick={() => setReportOpen(true)}
                    className="flex items-center justify-center gap-1.5 bg-black/20 border border-[var(--border-bg)] hover:border-red-500/40 py-2.5 px-3 rounded-xl text-xs font-bold text-gray-500 hover:text-red-400 transition" title="Report">
