...
function ChatContent() {
  ...
  return (
    <div className="flex h-full bg-[var(--card-bg)] border-t border-[var(--border-bg)] overflow-hidden fade-in">

      {/* ── Sidebar ── */}
      <div className={`${active && !showSidebar ? 'hidden md:flex' : 'flex'} w-full md:w-72 lg:w-80 flex-shrink-0 border-r border-[var(--border-bg)] flex-col bg-black/40`}>
        {/* Header */}
        <div className="px-4 py-4 border-b border-[var(--border-bg)]">
          <h2 className="font-black text-base text-white mb-3">Messages</h2>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-500" />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..."
              className="w-full bg-white/5 border border-[var(--border-bg)] rounded-xl pl-9 pr-3 py-2.5 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-brand transition" />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            <div className="text-center py-16 px-4 space-y-2">
              ...
              <p className="text-xs text-gray-600">Message a seller from any listing</p>
            </div>
          ) : filtered.map(c => {
            ...
            return (
              <button key={c.id} onClick={() => { setActive(c); setShowSidebar(false); }}
                ...
                } border-b border-[var(--border-bg)]/30`}>
                <div className="relative flex-shrink-0">
                  <div className="w-10 h-10 rounded-full bg-brand/15 border border-brand/25 flex items-center justify-center">
                    <span className="text-sm font-black text-brand">{name[0]?.toUpperCase() || '?'}</span>
                  </div>
                  {unread > 0 && (
                    <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 rounded-full bg-brand text-black text-[9px] font-black flex items-center justify-center">
                      {unread > 9 ? '9+' : unread}
                    </span>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center gap-1">
                    ...
                    <span className="text-[10px] text-gray-600 flex-shrink-0">{timeAgo(c.lastMessageAt)}</span>
                  </div>
                  <p className={`text-xs truncate mt-0.5 ${unread > 0 ? 'text-gray-400 font-medium' : 'text-gray-600'}`}>{preview}</p>
                </div>
              </button>
            );
          })}
      ...
      {/* ── Chat panel ── */}
      <div className={`${showSidebar && !active ? 'hidden md:flex' : 'flex'} flex-1 flex-col min-w-0`}>
        {active ? (
          <>
            {/* Chat header */}
            <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border-bg)] bg-black/30 flex-shrink-0">
              <button onClick={() => setShowSidebar(true)} className="md:hidden p-1.5 text-gray-400 hover:text-white rounded-lg transition">
                <ArrowLeft className="w-5 h-5" />
              </button>
              <div className="w-9 h-9 rounded-full bg-brand/15 border border-brand/25 flex items-center justify-center flex-shrink-0">
                <span className="text-sm font-black text-brand">{otherName(active, user?.id)[0]?.toUpperCase() || '?'}</span>
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-brand/70 flex items-center gap-1">
                  <Circle className="w-1.5 h-1.5 fill-brand/70" /> Online
                </p>
              </div>
              {active.orderId && (
                <Link href={`/orders/${active.orderId}`}
                  className="flex items-center gap-1.5 text-xs font-bold text-brand hover:text-brand-light transition bg-brand/10 border border-brand/20 px-3 py-1.5 rounded-lg flex-shrink-0">
              ...
              {grouped.map(({ day, msgs }) => (
                <div key={day}>
                  <div className="flex items-center gap-3 my-4">
                    ...
                    <span className="text-[10px] text-gray-600 font-semibold bg-[var(--card-bg)] px-2">{day}</span>
                    <div className="flex-1 h-px bg-[var(--border-bg)]" />
                  </div>
                    ...
                    {msgs.map(m => {
                      ...
                      return (
                        <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'} items-end gap-2`}>
                          ...
                          <div className="max-w-[75%] md:max-w-sm">
                            <div className={`px-4 py-2.5 text-sm leading-relaxed break-words ${
                              mine
                                ? 'bg-brand text-black font-medium rounded-2xl rounded-br-sm'
                                : 'bg-white/[0.06] border border-[var(--border-bg)] text-gray-100 rounded-2xl rounded-bl-sm'
                            }`}>
                              {m.content}
                            </div>
                            <div className={`flex items-center gap-1 mt-0.5 px-1 ${mine ? 'justify-end' : ''}`}>
                              ...
                              {mine && (
                                isPending
                                  ...
                                  : m.isRead
                                    ? <CheckCheck className="w-2.5 h-2.5 text-brand" />
            ...
            {/* Quick replies */}
            {showQuickReplies && (
              <div className="px-4 pb-2 flex flex-wrap gap-1.5 border-t border-[var(--border-bg)] pt-2">
                {QUICK_REPLIES.map(qr => (
                  <button key={qr} onClick={() => { setText(qr); setShowQuickReplies(false); inputRef.current?.focus(); }}
                    className="text-xs bg-brand/10 border border-brand/20 text-brand hover:bg-brand/20 px-3 py-1.5 rounded-full transition">
                    {qr.length > 40 ? qr.slice(0, 38) + '…' : qr}
                  </button>
                ))}
                <button onClick={() => setShowQuickReplies(false)} className="text-xs text-gray-500 hover:text-white p-1.5"><X className="w-3 h-3" /></button>
              </div>
            )}

            {/* Input bar */}
            <form onSubmit={send} className="flex items-center gap-2 px-3 py-3 border-t border-[var(--border-bg)] bg-black/30 flex-shrink-0">
              <button type="button" onClick={() => setShowQuickReplies(v => !v)} title="Quick replies"
                className="p-2 text-gray-500 hover:text-brand transition rounded-lg hover:bg-brand/10 flex-shrink-0">
                <Paperclip className="w-4 h-4" />
              </button>
              <input ref={inputRef} value={text} onChange={e => setText(e.target.value)} placeholder="Type a message..."
                className="flex-1 bg-white/5 border border-[var(--border-bg)] rounded-2xl px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-brand transition" />
              <button type="submit" disabled={!text.trim() || sending}
                className="w-10 h-10 bg-brand hover:bg-brand-light rounded-2xl flex items-center justify-center transition disabled:opacity-40 flex-shrink-0 shadow-lg shadow-brand/20">
          ...
          <div className="flex-1 flex flex-col items-center justify-center space-y-3 text-center p-8">
            ...
            <p className="text-xs text-gray-500">Or start a chat from any listing page.</p>
          </div>
        )}
      </div>
    </div>
      ...
      {/* ── Sidebar ── */}
      <div className={`${active && !showSidebar ? 'hidden sm:flex' : 'flex'} w-full sm:w-80 flex-shrink-0 border-r border-[var(--border-bg)] flex-col`}>
        <div className="px-4 py-4 border-b border-[var(--border-bg)] space-y-3">
          <h2 className="font-black text-base text-white">Messages</h2>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-500" />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search conversations..."
              className="w-full bg-background border border-[var(--border-bg)] rounded-xl pl-8 pr-3 py-2 text-xs focus:outline-none focus:border-brand transition" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filtered.length === 0 ? (
            ...
            return (
              <button key={c.id} onClick={() => { setActive(c); setShowSidebar(false); }}
                className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition border-b border-[var(--border-bg)]/40 ${isActive ? 'bg-brand/10 border-l-2 border-l-brand' : 'hover:bg-[var(--hover-bg)]/30'}`}>
                <div className="relative flex-shrink-0">
      ...
      <div className={`${showSidebar && !active ? 'hidden sm:flex' : 'flex'} flex-1 flex-col min-w-0`}>
        {active ? (
          <>
            {/* Chat header */}
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-[var(--border-bg)] bg-[var(--card-bg)]/80 backdrop-blur-sm">
                      ...
                      return (
                        <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'} items-end gap-2`}>
                          ...
                          <div className={`max-w-[72%] md:max-w-sm group`}>
                            <div className={`px-4 py-2.5 rounded-2xl text-sm shadow-sm ${mine ? 'bg-brand text-white rounded-br-sm' : 'bg-[var(--hover-bg)] border border-[var(--border-bg)]/60 rounded-bl-sm'}`}>
              ...
              <div className="px-4 pb-2 flex flex-wrap gap-2">
                {QUICK_REPLIES.map(qr => (
                  <button key={qr} onClick={e => { setText(qr); setShowQuickReplies(false); inputRef.current?.focus(); }}
                    className="text-xs bg-background border border-[var(--border-bg)] hover:border-brand/40 text-gray-300 hover:text-white px-3 py-1.5 rounded-full transition">
                    {qr.length > 40 ? qr.slice(0, 38) + '…' : qr}
                  </button>
                ))}
            ...
            {/* Input bar */}
            <form onSubmit={send} className="flex items-center gap-2 px-4 py-3 border-t border-[var(--border-bg)] bg-[var(--card-bg)]/80 backdrop-blur-sm">
              ...
              <input ref={inputRef} value={text} onChange={e => setText(e.target.value)} placeholder="Type a message..."
                className="flex-1 bg-background border border-[var(--border-bg)] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-brand transition" />
              <button type="submit" disabled={!text.trim() || sending}
...
export default function MessagesPage() {
  return (
    <div className="-mx-4 sm:-mx-6 lg:-mx-8 -my-6 h-[calc(100dvh-56px)]">
      <Suspense fallback={
        <div className="flex h-full items-center justify-center">
