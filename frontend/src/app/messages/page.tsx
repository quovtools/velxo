...
function ChatContent() {
  ...
  // Resolve which conversation to open based on the URL params.
  useEffect(() => {
        ...
        if (!target) {
          try {
            const created = await api.post<{ success: boolean; data: Conversation }>(
              ...
              { buyerId: buyerIdParam, sellerId: sellerIdParam },
            );
            if (created.success) {
              target = created.data;
  ...
  // Single real-time socket for the user's session. Messages are appended only
  // when they belong to the currently open conversation; the list is always
  // refreshed so unread badges stay accurate.
  useEffect(() => {
    if (!user) return;
  ...
  // When the active conversation changes, join its room, load messages and poll.
  useEffect(() => {
    if (!active) return;
  ...
  const send = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post<{ success: boolean; data: Message }>(
        `/messages/conversation/${active.id}/send`,
        { content: draft },
      );
      if (res.success) {
  ...
  return (
    <div className="flex h-[calc(100vh-180px)] min-h-[500px] bg-cardBg border border-borderBg rounded-2xl overflow-hidden fade-in">
      {/* Sidebar */}
      <div
        className={`${active && !showSidebar ? 'hidden sm:flex' : 'flex'} w-full sm:w-80 flex-shrink-0 border-r border-borderBg flex-col`}
      >
      ...
      {/* Chat panel */}
      <div
        className={`${showSidebar && !active ? 'hidden sm:flex' : 'flex'} flex-1 flex-col min-w-0`}
      >
        {active ? (
          <>
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-borderBg">
              <button
                ...
                className="sm:hidden p-1.5 text-gray-400 hover:text-white rounded-lg"
              >
              ...
              <div className="w-9 h-9 rounded-full bg-brand/10 border border-brand/20 flex items-center justify-center">
                <span className="text-sm font-bold text-brand-light">
                  {otherPartyName(active, user?.id)[0]?.toUpperCase() || '?'}
                </span>
              </div>
              <div className="flex-1 min-w-0">
              ...
              {messages.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div
                      className={`max-w-xs md:max-w-md px-4 py-2.5 rounded-2xl text-sm shadow-sm ${
                        mine
                          ? 'bg-brand text-white rounded-tr-sm'
                          : 'bg-hoverBg border border-borderBg rounded-tl-sm'
                      }`}
                    >
                      <p className="leading-relaxed break-words">{m.content}</p>
                      <p
                        className={`text-[9px] mt-1 ${
                          mine ? 'text-blue-200 text-right' : 'text-gray-500'
                        }`}
                      >
            ...
            <form
              onSubmit={send}
              className="flex items-center gap-2 px-4 py-3.5 border-t border-borderBg"
            >
              <input
                value={text}
