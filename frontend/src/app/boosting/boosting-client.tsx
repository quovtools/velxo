...
export default function BoostingClient() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Hire verified boosters to rank up your account — escrow-protected, delivered by piyrox sellers.
          </p>
        </div>
        {isSeller && (
          <button
      ...
      {!isSeller && user && (
        <div className="bg-purple-500/5 border border-purple-500/20 rounded-2xl p-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="flex-shrink-0 text-xs font-bold text-purple-300 hover:text-white transition"
          >
            Get started →
          </button>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-56 bg-cardBg border border-borderBg rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : gigs.length === 0 ? (
        <div className="text-center py-20 bg-cardBg border border-borderBg rounded-3xl">
          ...
          <p className="text-gray-400 font-semibold">
            No boosting services available{activeGame ? ` for ${activeGame}` : ''}.
          </p>
          {isSeller && (
            <button
              onClick={openCreate}
              className="mt-4 inline-flex items-center gap-2 bg-brand hover:bg-brand-dark text-white text-sm font-bold px-4 py-2 rounded-xl transition"
            >
            ...
            >
              ...
              <div className="p-3 flex-1 flex flex-col justify-between space-y-2">
                <div>
                  ...
                  {g.description && (
                    <p className="text-[11px] text-gray-400 mt-1 line-clamp-2">{g.description}</p>
                  )}
                  <div className="flex items-center gap-2 mt-2 text-[10px] text-gray-500">
                    ...
                    {g.deliveryTime && (
                      <span className="inline-flex items-center gap-1">
                        <Clock className="w-3 h-3" /> {g.deliveryTime}h
                      </span>
                    )}
                  </div>
                </div>
      ...
      {selected && (
        ...
        >
          <div
            className="bg-cardBg border border-borderBg rounded-2xl w-full max-w-md overflow-hidden"
          ...
          >
            <div className="relative h-36 bg-gradient-to-br from-purple-500/30 to-background flex items-center justify-center">
              ...
              <span className="absolute top-3 left-3 text-[9px] font-bold uppercase bg-purple-500 text-white px-1.5 py-0.5 rounded">
                {selected.accountType || 'Boost'}
              </span>
            </div>
            ...
            {purchasedId ? (
              <div className="p-6 text-center space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    onClick={closeDetail}
                    className="w-full text-gray-400 hover:text-white text-sm font-semibold py-2 transition"
                  >
                    Keep browsing
                  </button>
                </div>
              ...
              <div className="p-5 space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2 text-[11px] text-gray-500">
                    <span className="inline-flex items-center gap-1">
                      <ShieldCheck className="w-3 h-3 text-emerald-400" /> {selected.seller?.storeName || 'piyrox seller'}
                    </span>
                    {selected.deliveryTime && (
                      <span className="inline-flex items-center gap-1">
                        <Clock className="w-3 h-3" /> {selected.deliveryTime}h
                      </span>
                    )}
      ...
      {createOpen && (
        ...
        >
          <div
            className="bg-cardBg border border-borderBg rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-cardBg border-b border-borderBg px-5 py-4 flex items-center justify-between">
              <h3 className="text-white font-bold text-lg flex items-center gap-2">
                <Plus className="w-5 h-5 text-purple-400" /> Create Boosting Gig
              </h3>
              <button
                onClick={() => setCreateOpen(false)}
                className="w-8 h-8 rounded-full bg-background hover:bg-borderBg flex items-center justify-center text-white transition"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Diamond to Mythic boost"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    list="boost-rank-options"
                    placeholder={rankOptions.length ? `e.g. ${rankOptions[0]}` : 'e.g. Gold'}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    list="boost-rank-options"
                    placeholder={rankOptions.length ? `e.g. ${rankOptions[rankOptions.length - 1]}` : 'e.g. Diamond'}
                  />
                </div>
              </div>
              ...
              <button
                ...
                disabled={creating || !form.title || !form.description || !form.price}
                className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-brand hover:from-purple-700 hover:to-brand-dark text-white text-sm font-bold py-3 rounded-xl transition disabled:opacity-50"
              >
              ...
              <p className="text-[10px] text-gray-500 text-center">
                New gigs are submitted for admin review before going live.
              </p>
            </div>
          </div>
        </div>
